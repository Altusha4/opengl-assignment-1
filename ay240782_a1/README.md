...
All windows are 500x500 pixels.
...
* **Ellipse.** Parametric circle `x = cos(a)`, `y = sin(a)` with the y radius
  ...
  close the fan.
* **Triangle.** Three vertices placed at 90, 210 and 330 degrees on a circle.
  The corners are red, green and blue, and the rasteriser interpolates the rest.
* **Shaded circle.** Same fan construction, but the red channel of each
  perimeter vertex is `angle / (2 * pi)`, which produces a smooth ramp with a
  seam on the right where the angle wraps.
* **Concentric squares.** Six squares drawn largest to smallest, alternating
  white and black, each corner computed from the angles pi/4, 3pi/4, 5pi/4
  and 7pi/4.
...
* a custom 3x5 bitmap font where every letter is a grid of small squares, so
  the whole caption is drawn with a single `GL_TRIANGLES` call
...
GLSL 1.50). No extra libraries are needed there, since OpenGL and GLUT come
from the system frameworks and only the Xcode Command Line Tools are required.
